package ua.epam.final_project.controller.old.login;
...
public class UserRegistrationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (errorFlag.equals("0")) {
            ...
            if (userService.insertUser(UserDtoMapper.convertEntityIntoDto(user))) {
    ...
    private boolean validateLogin(String login, List<UserDto> userList) {
        return userList.stream()
                .anyMatch(x -> x.getLogin().equals(login));
    }

    private boolean validateEmail(String email, List<UserDto> userList) {
        return userList.stream()
