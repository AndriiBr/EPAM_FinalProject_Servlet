...
@Feature("Controller")
class OpenLoginFailPageCommandTest {

    @Mock
    private static SessionRequestContent content;

    public OpenLoginFailPageCommandTest() {
    ...
    void execute() {
        ExecutionResult result = new OpenLoginFailPageCommand().execute(content);

        assertEquals(Direction.FORWARD, result.getDirection());
        assertEquals(ResourceConfiguration.getInstance().getPage("auth.login.fail"), result.getPage());
    }
    ...
    void getAccessLevelList() {
        OpenLoginFailPageCommand command = new OpenLoginFailPageCommand();
